...
def criar_emprestimo(dados: EmprestimoCreate, db: Session = Depends(get_db)):
    return emprestimo_service.criar_emprestimo(
        db=db, usuario_id=dados.usuario_id, exemplar_id=dados.exemplar_id
    )


@router.post("/devolucao", response_model=EmprestimoResponse)
def devolver_emprestimo(dados: DevolucaoRequest, db: Session = Depends(get_db)):
...
def listar_emprestimos_do_usuario(usuario_id: int, db: Session = Depends(get_db)):
